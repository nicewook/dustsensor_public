...
if __name__ == "__main__":
	...
	import time
	import ds_v1 # import this script
	import sys
	...
	doc_ref = db.collection(u'dsensor')

	pi = pigpio.pi('localhost')
	s10 = ds_v1.sensor(pi, 4)  #  GPIO 4 - Seosor Pin 4 (PM10)
	s25 = ds_v1.sensor(pi, 2)  #  GPIO 2 - Seosor Pin 2 (PM25)
	...
	while True:
		...
		# calculate ug/m3
		# convert count/ft3 to ug/m3
		KMass25 = 0.00207
		PM25ugm3 = KMass25 * PM25count
		...
		PM25ugm3_temp = PM25ugm3

		if PM25ugm3_temp > 500.4:
			aqiPM25 = 500

		else:
			for breakpoint in cbreakpointspm25:
				if breakpoint[0] <= PM25ugm3_temp <= breakpoint[1]:
					Clow25 = breakpoint[0]
					Chigh25 = breakpoint[1]
					Ilow25 = breakpoint[2]
					Ihigh25 = breakpoint[3]
					aqiPM25 = (((Ihigh25-Ilow25)/(Chigh25-Clow25))*(PM25ugm3_temp-Clow25))+Ilow25     
		
		print("PM2.5 time: {}, Ratio: {:.3f}, Count: {}, ug/m3: {:.3f}, AQI: {}". 
			format(timestamp, r25, int(PM25count), PM25ugm3, int(aqiPM25)))

		# firebase upload
		doc_ref.add({
			u'timestamp': str(timestamp),
			u'pm25_ratio': str(round(r25,2)),
			u'pm25_count': str(int(PM25count)),
			u'pm25_ugm3': str(round(PM25ugm3,2)),
			u'pm25_aqi': str(int(aqiPM25)),
		})
